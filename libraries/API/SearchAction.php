...
class API_SearchAction
{
    ...
    public static function getCollections($type, $query_string)
    {
        ...
        $records->limit = intval(self::getParam('limit', API_Type::run($type, 'defaultLimit')));
        $records->filter = new StdClass;
        $records->id_fields = API_Type::run($type, 'getIdFields');
        $cmd->size = $records->limit;
        $cmd->from = ($records->page - 1) * $records->limit;

        $default_output_fields = API_Type::run($type, 'outputFields');
        ...
        $filter_fields = API_Type::run($type, 'filterFields');
        $default_sort_fields = API_Type::run($type, 'sortFields');
        ...
        if ($sort_fields) {
            ...
            foreach ($sort_fields as $field_name) {
                ...
                if (array_key_exists($field_name, $filter_fields) and $filter_fields[$field_name] !== '') {
                    ...
                    // 沒有對應的就用 getFieldMap
                    $es_field_name = API_Type::run($type, 'reverseField', [$field_name]);
                }
        ...
        if (self::getParams('q')) {
            ...
            $default_query_fields = API_Type::run($type, 'queryFields');
            ...
            $query_fields = array_map(function ($v) use ($type) {
                return API_Type::run($type, 'reverseField', [$v]);
            }, $query_fields);
            $cmd->query->bool->must[] = (object)[
                'query_string' => (object)[
            ...
            foreach (self::getParams('agg') as $agg) {
                ...
                while (count($agg_fields)) {
                    ...
                    if ($filter_fields[$agg_field] === '') {
                        $es_field = API_Type::run($type, 'reverseField', [$agg_field]);
                    } else {
        ...
        if (!is_null($output_fields)) {
            ...
            $cmd->_source = array_map(function($k) use ($type) {
                return API_Type::run($type, 'reverseField', [$k]);
            }, $output_fields);
        }
        ...
        $return_key = API_Type::run($type, 'getReturnKey');
        $records->{$return_key} = [];
        // 先掃一遍所有資料，把有在 aggMap 的欄位的 ID 儲存起來
        API_Type::run($type, 'checkHitRecords', [$obj->hits->hits]);
        foreach ($obj->hits->hits as $hit) {
            $records->{$return_key}[] = API_Type::run($type, 'buildData', [$hit->_source, $hit->_id, true, $hit->highlight ?? null]);
        }
        if (self::getParams('agg')) {
            $records->aggs = [];
            foreach (self::getParams('agg') as $agg) {
    ...
    public static function getItem($type, $ids, $sub, $query_string)
    {
        ...
        $relations = API_Type::run($type, 'getRelations');
        if ($sub[0] ?? false) {
            ...
            foreach ($rel['map'] as $source_key => $target_key) {
                $source_key = API_Type::run($type, 'reverseField', [$source_key]);
            ...
            if ($rel['type'] == '_function') {
                $data = API_Type::run($type, 'buildData', [$obj->_source, $obj->_id, false]);
                return API_Type::run($type, $rel['function'], [$data]);
            }
        ...
        $records->error = false;
        $records->id = $ids;
        $records->data = API_Type::run($type, 'buildData', [$obj->_source, $obj->_id, false]);
        $records->supported_relations = API_Type::run($type, 'getRelations');
        $records->relations = [];
    ...
    public static function mergeBucketByLevel($es_buckets, $agg_fields, $level, $values, $type)
    {
        ...
        foreach ($es_buckets as $es_bucket) {
            ...
            API_Type::run($type, 'addAggValue', [$agg_field, $es_bucket->key]);
    ...
    public static function getBuckets($es_buckets, $agg_fields, $level, $type)
    {
        ...
        $buckets = API_Type::run($type, 'handleAggValue', [$buckets, $agg_fields, $level]);
        return $buckets;
    }
}
