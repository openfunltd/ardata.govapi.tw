...
class API_Type
{
    ...
    /**
     * run 依照 $type 執行對應的 $method，Ex: API_Type::run('user', 'get', [1]) => API_Type_User::get(1)
     */
    public static function run($type, $method, $args = [])
    {
        $class = 'API_Type_' . API_Helper::ucfirst($type);
    ...
    {
        return strtolower(str_replace('API_Type_', '', get_called_class())) . 's';
    }
    ...
    public static function termSearch($class, $field, $type, $config)
    {
        ...
        if (!$query_field) {
            $query_field = API_Type::run($type, 'reverseField', [$config[0]]);
        }

        $output_field = API_Type::run($type, 'reverseField', [$config[1]]);
